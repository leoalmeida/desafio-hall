...
class ReleaseServiceTest {
    ...
    void testFindReleases() {
        ...
        when(releaseService.find(anyLong(), anyString(), any(EnvironmentEnum.class), any(StatusEnum.class))).thenReturn(releases);
        List<ReleaseResponseDto> result = releaseService.find(1L, "1.0", EnvironmentEnum.DEV, StatusEnum.CREATED);
        assertNotNull(result);
        verify(releaseService, times(1)).find(anyLong(), anyString(), any(EnvironmentEnum.class), any(StatusEnum.class));
    }
}
